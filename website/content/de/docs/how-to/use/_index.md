---
title: Anwendung benutzen
weight: 1
---

Rezepte für die Personen, die KitaManager täglich nutzen: Erzieher:innen, Kita-Leitungen, Vorstandsmitglieder. Wenn Sie eine Rolle als `manager`, `admin` oder `staff` in einer Organisation haben, sind die Aufgaben hier für Sie.

Für konto-bezogene Verwaltung (andere Nutzer:innen verwalten, Organisationseinstellungen ändern) siehe [Eine Organisation verwalten](../administer/).

## Tägliche Abläufe

- [Anwesenheit erfassen](track-attendance/)
- [Kind anlegen und Betreuungsvertrag ausstellen](add-a-child-and-issue-contract/)
- [Betreuungsvertrag eines Kindes aktualisieren](update-child-contract/)
- [Kita-Gutschein-Nummer zuweisen](assign-a-voucher/)
- [Kinder zwischen Bereichen verschieben](move-children-between-sections/)
- [Abgang eines Kindes erfassen](record-a-childs-departure/)
- [Personendaten eines Kindes oder einer Mitarbeiter:in aktualisieren](update-personal-data/)

## Personal

- [Mitarbeiter:in anlegen und Arbeitsvertrag erstellen](add-an-employee-and-create-contract/)
- [Arbeitsvertrag bei einer Änderung aktualisieren](update-employee-contract/)
- [Mitarbeiter:in zwischen Bereichen verschieben](move-employee-between-sections/)
- [Stufenaufstieg dokumentieren](promote-employee-step/)

## Förderung und Abrechnungen

- [Senatsabrechnung hochladen](upload-an-isbj-bill/)
- [Abweichung in einer Abrechnung untersuchen](investigate-a-bill-discrepancy/)
- [Abrechnungshistorie eines Kindes ansehen](view-a-childs-billing-history/)

## Planung

- [Prognose für Aufnahme- oder Personalplanung nutzen](use-the-forecast/)
- [Budgetposten verwalten](manage-budget-items/)

## Berichte

- [Bericht drucken](print-a-report/)

## Konto

- [Passwort ändern](change-password/)
- [Zwei-Faktor-Authentifizierung aktivieren](enable-2fa/)
- [Aktive Sitzungen verwalten](manage-active-sessions/)

## Daten

- [YAML-Daten importieren oder exportieren](import-and-export-yaml/)
