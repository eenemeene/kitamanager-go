...
- The contract edit dialog itself does not expose a section field. That's deliberate: changing the section on a contract that started in the past would silently rewrite historical staffing reports. The drag-and-drop and manual paths both produce a new contract from the effective date so the history stays correct.
