...
func TestStampColophon_AddsTextWithoutBreakingPDF(t *testing.T) {
	dir := t.TempDir()
	src := writeFixturePDF(t, dir, "src.pdf")
	target := filepath.Join(dir, "target.pdf")
	// Normalize through MergeFiles first — pdfcpu's stamping path expects
	// a context it can fully parse, and our hand-crafted minimal PDF
	// doesn't carry every optional table that the stamper consults. The
	// merge produces a fully-formed PDF that mirrors what the report
	// pipeline actually feeds StampColophon in production.
	if err := MergeFiles([]string{src}, target); err != nil {
		t.Fatalf("normalize fixture: %v", err)
	}

	const colophon = "Generated by report-pdf v0.27.0 · API v0.27.0 · 2026-04-27"
	if err := StampColophon(target, colophon); err != nil {
		t.Fatalf("StampColophon: %v", err)
	}
	// File still validates as a PDF after stamping.
	assertValidPDF(t, target)
}

func TestStampColophon_MissingFile(t *testing.T) {
	missing := filepath.Join(t.TempDir(), "does-not-exist.pdf")
	err := StampColophon(missing, "x")
	if err == nil {
		t.Fatal("expected error for missing PDF file")
	}
}
