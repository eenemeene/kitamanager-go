...
// The amendMode enum and determineAmendMode lived here: they inferred, from the
// contract's own dates, whether a PUT meant "edit this period" or "close it and
// create a successor". Nothing infers that now — the caller names the intent, so
// the same request body always does the same thing. The transaction shapes below
// are still shared by those named operations.
