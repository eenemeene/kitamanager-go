...
func TestGovernmentFundingHandler_CRUD(t *testing.T) {
	...
	handler := NewGovernmentFundingHandler(svc, createAuditService(db), importer.NewGovernmentFundingImporter(db, fundingStore))

	r := setupTestRouter()
	r.GET("/fundings", handler.List)
	r.GET("/fundings/:fundingId", handler.Get)
	r.POST("/fundings", handler.Create)
	r.PUT("/fundings/:fundingId", handler.Update)
	r.DELETE("/fundings/:fundingId", handler.Delete)
...
func TestGovernmentFundingHandler_UpdatePeriod_NoOverlap(t *testing.T) {
	...
	r := setupTestRouter()
	r.PUT("/fundings/:fundingId/periods/:periodId", handler.UpdatePeriod)
...
func TestGovernmentFundingHandler_DeletePeriod_Validation(t *testing.T) {
	...
	r := setupTestRouter()
	r.DELETE("/fundings/:fundingId/periods/:periodId", handler.DeletePeriod)
