...
func (h *ExportHandler) ExportEmployees(c *gin.Context) {
	...
	h.auditService.LogDataExport(c.Request.Context(), getUserID(c), getUserEmail(c), "employee", orgID, len(all), c.ClientIP())
...
func (h *ExportHandler) ExportChildren(c *gin.Context) {
	...
	h.auditService.LogDataExport(c.Request.Context(), getUserID(c), getUserEmail(c), "child", orgID, len(all), c.ClientIP())
